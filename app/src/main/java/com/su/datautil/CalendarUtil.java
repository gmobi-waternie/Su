 ...
 */
    ...
    public static int dayOfWeek(int y, int m, int d) {
            int w = 1; // 公历�?年一月一日是星期�?，所以起始�?�为星期�?
            y = (y - 1) % 400 + 1; // 公历星期值分�? 400 年循环一�?
            int ly = (y - 1) / 4; // 闰年次数
            ly = ly - (y - 1) / 100;
            ly = ly + (y - 1) / 400;
            int ry = y - 1 - ly; // 常年次数
    ...
    // 农历月份大小压缩表，两个字节表示�?年�?�两个字节共十六个二进制位数�?
    ...
    public String[] getYearTable() {
            ...
            table[ln] = blank;
            ln++;
            table[ln] = getTextLine(0, "##/## - 公历日期/农历日期�?(*)#�? - (�?)农历月第�?�?");
            ln++;
            return table;
    }
    ...
    private static String[] chineseMonthNames =
     ...
     private static String[] principleTermNames =
        {"大寒","雨水","春分","谷雨","夏滿","夏至","大暑","處暑","秋分","霜降", 
         "小雪","冬至"};
     private static String[] sectionalTermNames =
        {"小寒","立春","惊蟄","清明","立夏","芒种","小暑","立秋","白露","寒露",
         "立冬","大雪"};
